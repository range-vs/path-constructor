...
sub read {
	...
	if ($switch == 0) { 									#soc and pre-soc old format
		...
		if ($dh->find_chunk(CT_HEADER)) {
			...
			fail(__PACKAGE__.'::read', __LINE__, '$dh->find_chunk(CT_HEADER)', 'cannot find header chunk');
		}
		if ($dh->find_chunk(CT_DATA)) {
			...
			fail(__PACKAGE__.'::read', __LINE__, '$dh->find_chunk(CT_DATA)', 'cannot find data chunk');
		}
		$dh->close();
	} else {													#3120 and next
...
sub _read_cells {
	...
	$packet->length() % 0x6 == 0 or fail(__PACKAGE__.'::_read_cells', __LINE__, '$packet->data() % 0x6 == 0', 'bad CT_DATA chunk');

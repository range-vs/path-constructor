	...
	while (1) {
		...
		if ($index == 0) {
			...
			while (1) {
				...
				defined($index) or last;
				if ($index == 0) {
					$size == 4 or fail(__PACKAGE__.'::read', __LINE__, '$size == 4', 'point count size mismatch');
			...
			fail(__PACKAGE__.'::read', __LINE__, '$index == 0 || $index == 1', 'unexpected index '.$index);
		}
		$cf->r_chunk_close();
	}
}
sub read_points {
	...
	while (1) {
		...
		while (1) {
			...
			defined($index) or last;
			if ($index == 0) {
				$size == 4 or fail(__PACKAGE__.'::read_points', __LINE__, '$size == 4', 'point index size mismatch');
...
sub importing {
	...
	fail(__PACKAGE__.'::importing', __LINE__, 'defined $points', 'no points in path '.$section) unless defined $points;
	my %index_by_id;
	my $i = 0;

...
sub check_graph_version {
	...
	if ($switch <= 8) {
		...
		if ($version == 8) {
			...
			fail(__PACKAGE__.'::check_graph_version', __LINE__, '', 'wrong graph format');
		}
	}
}
sub decompose {
...
sub save_cross_tables {
	...
	foreach my $level (@{$self->{levels}}) {
		...
		my $fh = IO::File->new($fn, 'w') or fail(__PACKAGE__.'::save_cross_tables', __LINE__, '', "$! $fn\n");
		binmode $fh;
		fail(__PACKAGE__.'::save_cross_tables', __LINE__, '', 'cannot find cross table for level '.$level->{level_name}) unless defined $self->{raw_cross_tables}{$level->{level_name}};
...
sub error_handler {
	my $self = shift;
	print "Graph seems to be a $_[0] type. Reading it again...\n";
	$self->{gg_version} = $_[0];
	@{$self->{levels}} = ();
	$self->decompose();
}

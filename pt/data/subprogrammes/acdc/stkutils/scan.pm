...
use constant clsid_to_class => {
	...
	AI_DOG	 	=> 'se_monster',							# deprecated from build 1510
...
W_TZ_CHR => 'unknown',								# deprecated from build 1472
	...
	P_SKELET	=> 'cse_alife_object_physic',
...
#			$fhd->close();
		foreach my $section (%{$obj->{sections_hash}}) {
			next if !defined $obj->{sections_hash}{$section}{class};
			my $cse_class = clsid_to_class->{$obj->{sections_hash}{$section}{class}};
			$cse_class = $clsids_ini->value('clsids', $obj->{sections_hash}{$section}{class}) if (!defined $cse_class && defined $clsids_ini && $clsids_ini->is_value_exists('clsids', $obj->{sections_hash}{$section}{class}));
...
sub get_class {return clsid_to_class->{section_to_clsid->{$_[1]}}}
sub scan_system {
...
sub read_ini {
	...
	while (<$fh>) {
		...
		if (/^\[(.*)\]\s*:\s*(\w.*)?/) {
			$section = $1;
			fail(__PACKAGE__.'::read_ini', __LINE__, 'defined $self->{sections_hash}->{$section}', 'duplicate section found while reading '.$_[0]) if defined $self->{sections_hash}->{$section};
			...
			fail(
				...
				'duplicate section '.$section.' found while reading '.$_[0]) if (defined $self->{sections_hash}->{$section} && ($section ne 'postprocess_base'));
			push @{$self->{sections_list}}, $section;
			my %tmp = ();
			$self->{sections_hash}{$section} = \%tmp;
			next;			
